...
Status ServiceLayerServer::registeruser(ServerContext* context,
...
Status ServiceLayerServer::chirp(ServerContext* context,
  ...
  if (!sl_func_.user_exists(request->username())) {
    LOG(ERROR) << "User " << request->username()
  ...
  sl_func_.chirp(chirp, request->username(), request->text(),
                 request->parent_id());
  return Status::OK;
}

Status ServiceLayerServer::follow(ServerContext* context,
                                  const FollowRequest* request,
                                  FollowReply* reply) {
  // determine if the `username` and `to_follow` are valid before attempting to
  // follow
  if (!sl_func_.user_exists(request->username())) {
    return Status(StatusCode::INVALID_ARGUMENT, "User does not exist.");
...
Status ServiceLayerServer::read(ServerContext* context,
...
Status ServiceLayerServer::stream(ServerContext* context,
...
Status ServiceLayerServer::monitor(ServerContext* context,
