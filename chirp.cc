...
void print_read_output(
  ...
  if (chirps.size() > 0) {
    std::cout << "\"" << chirps[0].text() << "\""
              << " - " << chirps[0].username() << " ID: " << chirps[0].id()
              << std::endl;
  ...
  for (int i = 1; i < chirps.size(); i++) {
    ...
    std::cout << "\"" << chirps[i].text() << "\""
              << " - " << chirps[i].username() << " ID: " << chirps[i].id()
              << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  LOG(INFO) << "Client connection to server created at localhost:50002"
            << std::endl;
  // register flag provided, register username
  if (!FLAGS_register.empty()) {
    if (client.registeruser(FLAGS_register)) {
      std::cout << "Successfully registered user " << FLAGS_register
                << std::endl;
      LOG(INFO) << "Successfully registered user " << FLAGS_register
                << std::endl;
    } else {
      std::cout << "Attempt to register user " << FLAGS_register << " failed."
                << std::endl;
      LOG(ERROR) << "Attempt to register user " << FLAGS_register << " failed."
                 << std::endl;
      return 1;
    }
  }
  ...
  if (!FLAGS_user.empty()) {
    ...
    if (!FLAGS_follow.empty()) {
      if (client.follow(FLAGS_user, FLAGS_follow)) {
        ...
        LOG(INFO) << "User " << FLAGS_user << " successfully followed "
                  << FLAGS_follow << "." << std::endl;
      } else {
        std::cout << "Attempt for " << FLAGS_user << " to follow "
                  << FLAGS_follow << " failed." << std::endl;
        LOG(ERROR) << "Attempt for " << FLAGS_user << " to follow "
                   << FLAGS_follow << " failed." << std::endl;
        return 1;
      }
    }
